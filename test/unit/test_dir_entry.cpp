...
TEST_CASE("DDFS.DirEntry.calc_dir_entry_offsets.block 0")
{
	{
	...
	}
	{
		...
		const auto result =
			ddfs_calc_dir_entry_offsets(&calc_params, 3);
		...
		REQUIRE_EQ(result.first_cluster.block_on_device, 0);
		REQUIRE_EQ(
			result.first_cluster.offset_on_block,
			10 * (4 + sizeof(DDFS_DIR_ENTRY_ATTRIBUTES_TYPE) +
			      sizeof(DDFS_DIR_ENTRY_SIZE_TYPE)) +
	...
	}
	{
		...
		const auto cluster_first_block_on_device =
			...
			(calc_params.data_cluster_no +
			 calc_params.dir_logical_start);

		const auto result =
			ddfs_calc_dir_entry_offsets(&calc_params, 7);

		// =28 -> first block on cluster
		const auto expected_name_offset = 7 * 4;
		REQUIRE_EQ(result.name.block_on_device,
